...
class App extends React.Component {
	...
	componentDidMount () {
		...
		API.validateUser().then((data) => {
			if (!data.user) {
				if (this.props.history.location.pathname === '/signup') this.props.history.push(`/signup`);
				else if (this.props.history.location.pathname === '/') this.props.history.push(`/login`);
				...
				if (this.state.user.home_id === 1) {
					...
					if (this.state.user === undefined) {
						const text = [
							"These details doesn't seem to match any records.",
							'Please try again!'
						];
						swal({
							title   : 'Error!',
	...
	findHome = (homes, home_key) => {
		...
		if (newHome === undefined || home_key === '1') {
			const text = [
				'The home key you entered in not valid.',
	...
	updateUser = (userNewInfo) => {
		this.updateThisUser(this.state.user, userNewInfo).then((user) =>
			this.setState({ user }, this.componentDidMount(), this.props.history.push(`/dashboard`))
		);
		// .then(user => {
		//   this.setState({ user }, () => {
	...
	generateRandomHomeKey = () => {
		...
		const text = [
			generateId(16),
			'Please copy and then paste the key in the home key field of the form'
		];
		swal({
				...
				{this.state.user ? (
							...
							render={() => (
								<div>
									<Navbar
										...
										redirectToMovingHome={this.redirectToMovingHome}
									/>
									<Dashboard
										user={this.state.user}
										data={this.state.data}
										redirectToHomeProfile={this.redirectToHomeProfile}
										width={this.state.width}
										addNewEssential={this.addNewEssential}
										deleteEssential={this.deleteEssential}
										updateEssential={this.updateEssential}
										deleteTask={this.deleteTask}
										addNewTask={this.addNewTask}
										updateTask={this.updateTask}
										addTaskToCurrentUser={this.addTaskToCurrentUser}
										addNewBill={this.addNewBill}
										removeBill={this.removeBill}
										updateBillSplit={this.updateBillSplit}
										addOtherBillsToCurrentUser={this.addOtherBillsToCurrentUser}
									/>
								</div>
							)}
						/>
							...
							render={() => (
								<div>
									...
									<Profile
										...
										redirectToCookiePolicy={this.redirectToCookiePolicy}
										width={this.state.width}
									/>
								</div>
							)}
							...
							render={() => (
								<div>
									...
									<Home
										user={this.state.user}
										data={this.state.data}
										submitNewHomeDetails={this.getHomesData}
										submitNewHomeKey={this.submitNewHomeKey}
										generateRandomHomeKey={this.generateRandomHomeKey}
									/>
								</div>
							)}
						/>
						...
						<Route
							...
							render={() => (
								<div>
									...
									/>
									<MovingHome
										moveToNewHome={this.moveToNewHome}
										data={this.state.data}
										createNewHome={this.getHomesDataForNewHome}
										generateRandomHomeKey={this.generateRandomHomeKey}
										width={this.state.width}
									/>
								</div>
					...
					<div>
						<Route
							exact
							path="/login"
							render={() => <LogInForm user={this.state.user} submit={this.logIn} />}
						/>
						<Route exact path="/signup" render={() => <SignUpForm submit={this.signUp} />} />
					</div>
				)}
			</div>
		);
