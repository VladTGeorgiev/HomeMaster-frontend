...
class BillsCard extends React.Component {
	...
	render () {
		...
		const userBills = user_bill_splits.map(
			(bill_split) => bills.filter((bill) => bill_split.bill_id === bill.id)[0]
		);

		const other_bill_splits_all = all_bill_splits.filter((bill_split) => bill_split.user_id !== user.id);

		const other_bill_splits = other_bill_splits_all.filter(
			(obs) => !user_bill_splits.map((ubs) => ubs.bill_id).includes(obs.bill_id)
		);

		const unique_other_bill_splits = other_bill_splits
			.map((e) => e.bill_id)
			.map((e, i, final) => final.indexOf(e) === i && i)
			.filter((e) => other_bill_splits[e])
			.map((e) => other_bill_splits[e]);

		const otherBillsAll = unique_other_bill_splits.map(
			(bill_split) => bills.filter((bill) => bill_split.bill_id === bill.id)[0]
		);
					...
					{otherBills === undefined ? (
							...
							<Grid.Column style={{ width: '90vw' }}>
								...
								<Table.Body>
									...
									<Table.Row>
											...
											<Header as="h5">
												<Header.Content>
													...
													{userBills.map((userBill) => (
														<p onClick={() => this.userBillDetails(userBill)}>
															{userBill.name}
															<Icon name="info" />
														</p>
													))}
										...
										<Table.Cell />
										<Table.Cell>
											{user_bill_splits.map((bill_split) => (
												<p>
													<Checkbox
														toggle
														checked={bill_split.paid}
														onChange={() => updateBillSplit(bill_split, this.props.user)}
													/>
												</p>
											))}
										</Table.Cell>
						...
						<Grid textAlign="center" verticalAlign="top">
							<Grid.Column style={{ width: '90vw' }}>
									...
									<Table.Row>
										<Table.Cell>
											<Header as="h5">
												<Header.Content>
													{/* {all_tasks.map(task => <Image src={task.img} size='small' />)} */}
													{otherBills.map((otherBill) => (
														<p onClick={() => this.otherUsersBillDetails(otherBill)}>
															{otherBill.name}
															<Icon name="info" />
														</p>
													))}
										...
										<Table.Cell />
										<Table.Cell>
											{unique_other_bill_splits.map((unique_other_bill_split) => (
												<p>
													<Checkbox
														toggle
														onChange={this.message}
														checked={unique_other_bill_split.paid}
													/>
												</p>
											))}
										</Table.Cell>
										...
										<Table.Cell>
											{otherBills.map((otherBill) => (
												<Button
													...
													size="small"
													onClick={(e) =>
														addOtherBillsToCurrentUser(otherBill, this.props.user)}
												>
													Take part
												</Button>
											))}
								...
								<Button color="yellow" size="medium" onClick={() => addNewBillForm()}>
									Add more bills
								</Button>
							</Grid.Column>
						</Grid>
								...
								<Table basic="very" celled collapsing>
									...
									<Table.Body>
										...
										<Table.Row>
												...
												{user_bill_splits.map((bill_split) => (
													<p>
														<Checkbox
															...
															checked={bill_split.paid}
															onChange={() =>
																updateBillSplit(bill_split, this.props.user)}
														/>
													</p>
												))}
											</Table.Cell>
											...
											<Table.Cell>{userBills.map((bill) => <p>{bill.provider}</p>)}</Table.Cell>
											<Table.Cell>{userBills.map((bill) => <p>{bill.date_due}</p>)}</Table.Cell>
											<Table.Cell>
												{userBills.map((bill) => (
													<Button
														color="red"
								...
								<Table basic="very" celled collapsing>
									...
									<Table.Body>
										...
										<Table.Row>
											...
											<Table.Cell>{otherBills.map((bill) => <p>{bill.provider}</p>)}</Table.Cell>
											<Table.Cell>
												{otherBills.map((otherBill) => <p>{otherBill.date_due}</p>)}
											</Table.Cell>
											<Table.Cell>
