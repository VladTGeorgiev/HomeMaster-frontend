					...
					<Grid.Column style={{ width: '90vw' }}>
						...
						<Table.Body>
							...
							<Table.Row>
								<Table.Cell>
									<Header as="h5">
										<Header.Content>
											...
											{tasks.map((task) => (
												<p onClick={() => this.userTaskDetails(task)}>
													{task.name}
													<Icon name="info" />
												</p>
											))}
									...
									{tasks.map((task) => (
										<p>
											<Checkbox
												...
												onChange={() => updateTask(task, this.props.user)}
											/>
										</p>
									))}
								</Table.Cell>
								...
								<Table.Cell>
									{tasks.map((task) => (
										<Button color="red" fluid size="small" onClick={(e) => removeTask(e, task)}>
											Remove
										</Button>
									))}
								</Table.Cell>
							...
							<Table.Row>
								<Table.Cell>
									<Header as="h5">
										<Header.Content>
											...
											{otherTasks.map((task) => (
												<p onClick={() => this.otherUsersTaskDetails(task)}>
													{task.name}
													<Icon name="info" />
												</p>
											))}
								...
								<Table.Cell>
									{otherTasks.map((task) => (
										<p>
											<Checkbox toggle onChange={this.message} checked={task.completed} />
										</p>
									))}
								</Table.Cell>
								<Table.Cell />
								<Table.Cell>
									{otherTasks.map((task) => (
										<Button
											...
											fluid
											size="small"
											onClick={(e) => this.props.addTaskToCurrentUser(task, this.props.user)}
										>
											<div className="comfortaa">Take over</div>
										</Button>
									))}
								</Table.Cell>
							</Table.Row>
						...
						<Button color="olive" size="medium" onClick={() => this.props.addNewTaskForm()}>
							<div className="comfortaa">Add more tasks</div>
						</Button>
					</Grid.Column>
				</Grid>
			);
						...
						<Table basic="very" celled collapsing>
							...
							<Table.Body>
								...
								<Table.Row>
									...
									<Table.Cell>{tasks.map((task) => <p>{task.description}</p>)}</Table.Cell>
									<Table.Cell>
										{tasks.map((task) => (
											<p>
												<Checkbox
									...
									<Table.Cell>{tasks.map((task) => <p>{this.props.user.first_name}</p>)}</Table.Cell>
									<Table.Cell>{tasks.map((task) => <p>{task.day}</p>)}</Table.Cell>
									<Table.Cell>
										{tasks.map((task) => (
											<Button color="red" fluid size="small" onClick={(e) => removeTask(e, task)}>
												Remove
								...
								<Table.Row>
									...
									<Table.Cell>{otherTasks.map((task) => <p>{task.description}</p>)}</Table.Cell>
									<Table.Cell>
										{otherTasks.map((task) => (
											<p>
												<Checkbox toggle onChange={this.message} checked={task.completed} />
									...
									<Table.Cell>
										{otherTasks.map((task) => (
											<p>{users.find((user) => user.id === task.user_id).first_name}</p>
										))}
									</Table.Cell>
									<Table.Cell>{otherTasks.map((task) => <p>{task.day}</p>)}</Table.Cell>
									<Table.Cell>
										{otherTasks.map((task) => (
											<Button
												color="yellow"
